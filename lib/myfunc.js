...
exports.jam = (numer, options = {}) => {
    ...
    let timeZone = options.timeZone || 'Africa/Lagos'
    let jam = options?.timeZone ? moment(numer).tz(timeZone).format(format) : moment(numer).tz('Africa/Lagos').format(format)
    ...
    return new Promise((resolve, reject) => {
        if (/http/.test(path)) {
            axios.get(path)
                ...
                .catch(err => reject('Error fetching media: ' + err.message))
            ...
            reject('Invalid path or buffer provided')
        }
    })
}
...
exports.smsg = (XeonBotInc, m, store) => {
    ...
    if (m.message) {
        ...
        if (m.quoted) {
             ...
             * @param {Boolean} forceForward - Force forward
     ...
     * @param {String|Object} text - Text or buffer to send
    ...
    /**
     * Copy this message
     * @returns {Object} Copied message object
     */
...
exports.reSize = (buffer, width, height) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            var resized = await image.resize(width, height).getBufferAsync(Jimp.MIME_JPEG)
            resolve(resized)
        } catch (error) {
            reject(error)
        }
